...
impl<'s> System<'s> for BounceSystem {
    ...
    fn run(&mut self, (mut balls, paddles, transforms): Self::SystemData) {
        ...
        //
        // We also check for the velocity of the ball every time, to prevent multiple
        // collisions from occuring.
        for (ball, transform) in (&mut balls, &transforms).join() {
            ...
            for (paddle, paddle_transform) in (&paddles, &transforms).join() {
                ...
                // lowest coordinates, and adding the ball radius to the highest ones. The
                // ball is then within the paddle if its center is within the larger wrapper
                // rectangle.
                if point_in_rect(
                    ball_x,
                    ball_y,
